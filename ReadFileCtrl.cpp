/*******************************************************************************
 * File: logScanner/ReadFileCtrl.cpp                                           *
 * Created:     Oct-01-25                                                      *
...
std::string ReadFileCtrl::m_TIMESTAMP_FILES_REGEX = "[[:digit:]]{" + std::to_string(LogManager::TIMESTAMP_SIZE) + "}-[[:alpha:]]{1,}\\.ctrl";

ReadFileCtrl::ReadFileCtrl(std::string logFilePath) :
	m_logFilePath(logFilePath),
	m_ctrlBaseName(m_CONTROL_FILES_PATH + logFilePath),
	m_fileOrSegmentEOF(0),
	m_nextLogline2Read(0),
	m_nextCatchup2Read(0),
	m_doneCatchingUp(false),
	m_lastLoglinePosWritten(0),
	m_lastCatchupPosWritten(0)
{
	std::filesystem::create_directories(m_ctrlBaseName);
}

ReadFileCtrl::~ReadFileCtrl()
{
}

bool ReadFileCtrl::initReadLogThread()
{
	...
	LogManager::getInstance()->consoleMsg(("Opening logfile for reading beyond EOF\t=>\t" + m_logFilePath).c_str());
	...
	{
		LogManager::getInstance()->consoleMsg(("*** FATAL *** unable to open logfile for reading beyond EOF " + m_logFilePath).c_str());
	}

	return m_ifsReadLog.is_open();
}

void ReadFileCtrl::stopReadLogThread()
{
	m_ifsReadLog.close();
}

bool ReadFileCtrl::initCatchUpThread()
{
	...
	LogManager::getInstance()->consoleMsg(("Opening (Catch UP) logfile\t=>\t" + m_logFilePath).c_str());
	...
	{
		LogManager::getInstance()->consoleMsg(("*** FATAL *** unable to open logfile (Catch UP) " + m_logFilePath).c_str());
	}

	return m_ifsCatchUp.is_open();
}

void ReadFileCtrl::stopCatchUpThread()
{
	m_ifsCatchUp.close();
}

bool ReadFileCtrl::fileHasBackLog()
{
	// What is a backlog?
	// How do we detect it?
...
//LogManager::getInstance()->consoleMsg(("DBG_INSIDE_fileHasBackLog").c_str());
	// m_fileOrSegmentEOF was set to EOF by ReadFileCtrl::initReadLogThread()
	if(m_fileOrSegmentEOF == 0)
	{
		...
		//      => Thread 1 starts reading from begining of file.
		//      => Thread 2 doesn't start.
		LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 1: Logfile " + m_logFilePath).c_str());
	...
	{
		// NON empty logfile.
		LogManager::getInstance()->consoleMsg(("fileHasBackLog => Logfile " + m_logFilePath + " size is " + std::to_string(m_fileOrSegmentEOF)).c_str());
		...
		if(!gotNextLogline2Read)
		{
			...
			//      => Thread 2 starts reading from begining of file to "fileOrSegmentEOF.ctrl".
			//      => hasBacklog => TRUE;
			LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 2: Logfile " + m_logFilePath).c_str());
		...
		{
			LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3: Logfile " + m_logFilePath).c_str());
			...
			{
				LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3.1: Logfile " + m_logFilePath).c_str());
				...
				if(m_fileOrSegmentEOF == m_nextLogline2Read) // Nothing to do...
				{
					...
					//          => Thread 1 starts reading from EOF.
					//          => Thread 2 doesn't start.
					LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3.1.1: Logfile " + m_logFilePath).c_str());
				}
				else if(m_fileOrSegmentEOF > m_nextLogline2Read)
				{
					...
					//          => Thread 2 starts reading from "nextCatchup2Read.ctrl" up to "fileOrSegmentEOF.ctrl".
					//          => hasBacklog => TRUE;
					LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3.1.2: Logfile " + m_logFilePath).c_str());
				...
				else
				{
					// This is a problem...
					// How did I get here?     :(
					LogManager::getInstance()->consoleMsg(("*** WARNING ***  => fileHasBackLog => Case 3.1: Logfile " + m_logFilePath +
			...
			{
				LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3.2: Logfile " + m_logFilePath).c_str());
				...
				if(m_fileOrSegmentEOF == m_nextLogline2Read) // Nothing to do...
				{
					...
					//          => Thread 1 starts reading from EOF.
					//          => Thread 2 starts reading from "nextCatchup2Read.ctrl" up to "fileOrSegmentEOF.ctrl".
					LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3.2.1:Logfile " + m_logFilePath).c_str());
				}
				else if(m_fileOrSegmentEOF > m_nextLogline2Read)
				{
					//     Case 3.2,2: EOF > "fileOrSegmentEOF.ctrl" ->  NEW LINES HAVE been appended.
					...
					//          => Thread 2 starts reading from "nextCatchup2Read.ctrl" up to "fileOrSegmentEOF.ctrl".
					//          => hasBacklog => TRUE;
					LogManager::getInstance()->consoleMsg(("fileHasBackLog => Case 3.2.2:Logfile " + m_logFilePath).c_str());
				...
				else
				{
					// This is a problem...
					// How did I get here?     :(
					LogManager::getInstance()->consoleMsg(("*** WARNING ***  => fileHasBackLog => Case 3.2: Logfile " + m_logFilePath +
					                                       " m_fileOrSegmentEOF = " + std::to_string(m_fileOrSegmentEOF) +
					                                   " <=> m_nextLogline2Read = " + std::to_string(m_nextLogline2Read)).c_str());
				}
			}
			else
			{
				// This is a problem...
				// How did I get here?     :(
				LogManager::getInstance()->consoleMsg(("*** WARNING ***  => fileHasBackLog => Case 3: Logfile " + m_logFilePath +
...
bool ReadFileCtrl::getNextLogLine(std::string &line)
{
...
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getNextLogLine -> EOF = " + std::string(m_ifsReadLog.eof() ? "T" : "F") + " => LINE '" + line + "'").c_str()); //FIXTHIS!!!
	return !m_ifsReadLog.eof();
}

bool ReadFileCtrl::getCatchupLine(std::string &line)
{
	// This function should NEVER see EOF!
	// In ReadFileCtrl::initReadLogThread() we do:
	    // m_ifsReadLog.open(m_logFilePath, std::ios::in);
	    // Set the reading cursor to EOF and store the size of the file
	    // m_ifsReadLog.seekg(0, m_ifsReadLog.end);
	    // m_fileOrSegmentEOF = m_ifsReadLog.tellg();
	...
	m_nextCatchup2Read = m_ifsCatchUp.tellg();
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getCatchupLine -> m_doneCatchingUp = " + (m_doneCatchingUp ? "T" : "F") + " => m_ifsCatchUp.eof() = " + (m_ifsCatchUp.eof() ? "T" : "F")).c_str()); //FIXTHIS!!!
	while(!m_doneCatchingUp && !gotLineToGo && !m_ifsCatchUp.eof())
	{
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getCatchupLine -> TOP LOOP.").c_str()); //FIXTHIS!!!
		if(m_nextCatchup2Read < m_fileOrSegmentEOF)
		{
			// Still reading this segment.
			std::getline(m_ifsCatchUp, line);
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getCatchupLine -> EOF = " + (m_ifsCatchUp.eof() ? "T" : "F") + " => LINE '" + line + "'").c_str()); //FIXTHIS!!!
			gotLineToGo = true;
		}
		else
		{
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getCatchupLine -> NXY_SEG = "  + " => LINE '" + line + "'").c_str()); //FIXTHIS!!!
...
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getCatchupLine -> END_ALL_SEG").c_str()); //FIXTHIS!!!
...
void ReadFileCtrl::writePosOfNextLogline2Read()
{
	boost::mutex::scoped_lock lock(m_writeMutexNextLogline);
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::writePosOfNextLogline2Read => " + m_ctrlBaseName + m_NEXTLOGLINE2READ_CTRL + " => " + std::to_string(m_nextLogline2Read)).c_str());
...
void ReadFileCtrl::writePosOfNextCatchup2Read()
{
	boost::mutex::scoped_lock lock(m_writeMutexNextCatchup);
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::writePosOfNextCatchup2Read => " + m_ctrlBaseName + m_NEXTCATCHUP2READ_CTRL + " => " + std::to_string(m_nextCatchup2Read)).c_str());
	// This function is called at the end of LogScanner::insertIntoSeenTable which
	// in turn is called after all the regexps for that line have been evaluated.
	// There is a race condition to write the offset of the last "read",
	// because rexgeps are evaluated asynchronically, so...
...
bool ReadFileCtrl::eofCatchUp()
{
	return m_doneCatchingUp;
}

bool ReadFileCtrl::refreshCtrlValOf(std::streampos &ctrlVal)
{
...
bool ReadFileCtrl::setCtrlValOf(std::string ctrlFile, std::streampos &ctrlVal)
{
	std::ofstream ofs(ctrlFile, std::ios_base::trunc);
	bool          fileOpened = false;
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::setCtrlValOf => IN => '"         + ctrlFile + "'" + std::to_string(ctrlVal)).c_str()); //FIXTHIS!!!
	if(ofs.is_open())
	{
		ofs << ctrlVal << std::endl;
		ofs.close();
//LogManager::getInstance()->consoleMsg(("DBG_WRITTEN_ReadFileCtrl::setCtrlValOf => IN => '" + ctrlFile + "'" + std::to_string(ctrlVal)).c_str()); //FIXTHIS!!!
		fileOpened = true;
	}
...
bool ReadFileCtrl::getCtrlValOf(std::string ctrlFile, std::streampos *ctrlVal)
{
	std::ifstream ifs(ctrlFile, std::ios::in);
	bool          fileOpened = false;
//LogManager::getInstance()->consoleMsg(("DBG_ReadFileCtrl::getCtrlValOf => IN => '" + ctrlFile + "'").c_str()); //FIXTHIS!!!
	if(ifs.is_open())
	{
		std::string line;
...
void ReadFileCtrl::removeAllControlFiles()
{
	...
	remove((m_ctrlBaseName + m_FILEORSEGMENTEOF_CTRL).c_str());
	remove((m_ctrlBaseName + m_NEXTLOGLINE2READ_CTRL).c_str());
	remove((m_ctrlBaseName + m_NEXTCATCHUP2READ_CTRL).c_str());
}
/*END*/
